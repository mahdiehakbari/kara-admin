    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div
              className={`bg-(--surface) rounded-2xl shadow-lg relative transition-colors`}
            >
              {title && (
                <div className='p-4 border-b border-(--border-color) flex justify-between items-center'>
                  ...
                  <Image
                    ...
                    width={24}
                    height={24}
                    className='cursor-pointer hover:opacity-80 icon-invert'
                    onClick={onClose}
                  />
                </div>
              )}
          ...
          <motion.div
            className={`sm:hidden fixed bottom-0 left-0 right-0 z-50 bg-(--surface) rounded-t-2xl shadow-lg max-h-[90vh] overflow-y-auto transition-colors`}
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
            transition={{
